...
namespace FrontDesk.Api.AppointmentEndpoints
{
          ...
          .WithActionResult<ListAppointmentResponse>
        {
                ...
                // 43:21
                ///<summary>
                //when we pass in the request we're specifying a schedule ID and if that
                // schedule ID is missing or empty then we're going to return not found from
                // this API otherwise it uses the schedule by ID
                // with appointment spec to encapsulate the query that it's going to use on the page
                // in question we only want the appointments for one day it's worth
                // noting that this specification does not perform any filtering by date it returns
                // all appointments for this schedule
                ///</summary>
                  ...
                  CancellationToken cancellationToken)
                {
                        ...
                        schedule = await _scheduleRepository.FirstOrDefaultAsync(spec);
                        ...
                        foreach (var appt in myAppointments)
                        {
                                ...
                                var client = await _clientRepository.FirstOrDefaultAsync(clientSpec);
