...
namespace BusinessManagement.Api
{
  public class RabbitMQService : IHostedService
  {
    ...
    private const string exchangeName = MessagingConstants.Exchanges.FRONTDESK_BUSINESSMANAGEMENT_EXCHANGE;
    private const string queuein = MessagingConstants.Queues.FDCM_BUSINESSMANAGEMENT_IN;
    private const string queueout = MessagingConstants.Queues.FDCM_FRONTDESK_IN;

    // Manually Run RabbitMQ
    // docker run --rm -it --hostname first-encounter-ddd-sample-rabbit -p 15672:15672 -p 5672:5672 rabbitmq:3-management
